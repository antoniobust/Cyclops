package com.mdmobile.cyclops.apiManager.api;


public class ApiTypes {
    //API types
    public static final String groupsApi = "api/devicegroups";
    public static final String devicesApi = "api/devices";
    public static final String deviceSearch = devicesApi + "/search";
    public static final String directoriesResourceApi = "api/directories";
    public static final String usersResource = "api/security/users";
    public static final String serverResource = "api/servers";
}
